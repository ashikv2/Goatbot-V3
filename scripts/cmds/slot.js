module.exports = {
  ...
  onStart: async function ({ message, event, args, usersData }) {
    ...
    const allSlots = [slot1, slot2, slot3, slot4, slot5];
    const counts = {};
    allSlots.forEach(s => counts[s] = (counts[s] || 0) + 1);
    const maxMatch = Math.max(...Object.values(counts));

    if (allSlots.every(s => s === "7️⃣")) {
      winnings = bet * 10;
      ...
      winnings = bet * 8;
      outcome = `💰 𝗝𝗔𝗖𝗞𝗣𝗢𝗧! 5 matching symbols!`;
      winType = "💫 𝗕𝗜𝗚 𝗪𝗜𝗡";
    }
    else if (maxMatch === 4) {
      winnings = bet * 4;
      outcome = `✨ 𝗡𝗜𝗖𝗘! 4 matching symbols!`;
      winType = "🌟 𝗪𝗜𝗡";
    }
    else if (maxMatch === 3) {
      winnings = bet * 2;
      outcome = `🎯 𝗟𝗨𝗖𝗞𝗬! 3 matching symbols!`;
      winType = "🍀 𝗦𝗠𝗔𝗟𝗟 𝗪𝗜𝗡";
    }
    ...
    // 5-slot stylish ASCII box (new design)
    const slotBox =
      "╔═✦✧═══════✧✦═╗\n" +
      "     🎰 𝗦𝗟𝗢𝗧 𝗠𝗔𝗖𝗛𝗜𝗡𝗘 🎰\n" +
      "╠═✦✧═══════✧✦═╣\n" +
    ...
    const messageContent =
      `${slotBox}\n` +
      `${winnings >= 0 ? `     ⚡ Congrats! 🎉` : `     ⚡ Aaah! Try again! ⚡`}\n` +
      "╠═✦✧═══════✧✦═╣\n" +
      `     ${resultColor} 𝗥𝗘𝗦𝗨𝗟𝗧: ${resultText}\n` +
      `     💰 BALANCE: ${formatMoney(finalBalance)}\n` +
      "╠═✦✧═══════✧✦═╣\n" +
      "     💎 TIP: Bet bigger → Win bigger! 💎\n" +
      "╚═✦✧═══════✧✦═╝";
