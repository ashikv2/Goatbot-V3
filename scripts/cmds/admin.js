            ...
            case "-l": {
                const getNames = await Promise.all(
                    config.adminBot.map(uid =>
                        usersData.getName(uid).then(name => ({ uid, name: name || "null" }))
                    )
                );

                // Main Owner Profile (first admin)
                const owner = getNames[0];

                // Generate Admin Boxes
                const adminBoxes = getNames.map(({ uid, name }, index) => {
                    return `
╭──────────────────────╮
│ 🌟 Admin ${index + 1}: ${name}
│ 🆔 ${uid}
...
◆ ғʙ ʟɪɴᴋ:
https://www.facebook.com/profile.php?id=${owner.uid}

━━━━━━━━━━━━━━━━━━━━━━

🛡️ B O T  •  A D M I N  •  L I S T 🛡️
